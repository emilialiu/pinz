...
(function(){
	if($.Datatype){
		$.extend($.Tipmsg.w,{
			...
			"notvalued":"不能含有特定值！",
			"idcard":"身份证号码不对！",
			'file_doc':"请上传.doc.docx文件",//添加人ssm
			'file_xls':"请上传.xls.xlsx文件",
			'file_img':"请上传.jpg.jpeg.gif.bmp文件"
		});
		...
		$.extend($.Datatype,{
			...
			*/
			...
			},"file_doc":function(gets,obj,curform,datatype){
				if($.trim($(obj).val())==""){
					return true;
				}
				var file=$(obj).val().substring($(obj).val().lastIndexOf("."),$(obj).val().length);
				if(file.toLowerCase()=='.doc' || file.toLowerCase()=='.docx'){
					return true;
				}
				return "请上传后缀.doc或者.docx文件";
			},"file_xls":function(gets,obj,curform,datatype){
				if($.trim($(obj).val())==""){
					return true;
				}
				var file=$(obj).val().substring($(obj).val().lastIndexOf("."),$(obj).val().length);
				if(file.toLowerCase()=='.xls' || file.toLowerCase()=='.xlsx')
					return true;
				return "请上传后缀.xls或者.xlsx文件";
			},"file_img":function(gets,obj,curform,datatype){
				if($.trim($(obj).val())==""){
					return true;
				}
				var file=$(obj).val().substring($(obj).val().lastIndexOf("."),$(obj).val().length);
				if(file.toLowerCase()=='.gif' || file.toLowerCase()=='.jpg' || file.toLowerCase()=='.jpeg' || file.toLowerCase()=='.bmp'|| file.toLowerCase()=='.png')
					return true;
				return "请上传正确图片格式的文件";
			}
		});
