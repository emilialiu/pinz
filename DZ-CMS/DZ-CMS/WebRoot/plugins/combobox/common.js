...
ErrorHelper.invalidArgument = function(argName) {
...
ErrorHelper.notImplemented = function() {
...
ArrayHelper.isArray = function(obj) {
    return typeof obj === "object" && typeof obj.length === "number";
}

ArrayHelper.indexOf = function(array, item, start, length) {
...
/// <param name="length" type="Number" optional="true"></param>
    if (ArrayHelper.isArray(array)) {
        ...
        else if (start < 0) ErrorHelper.invalidArgument("start");
        ...
        ErrorHelper.invalidArgument("array");
...
ArrayHelper.forEach = function(array, func, state) {
    var rtv;
...
ArrayHelper.where = function(array, func, state, max) {
    var result = [];
    ...
    ArrayHelper.forEach(array, test);

    return result;
}

ArrayHelper.first = function(array, func, state) {
    var r = ArrayHelper.where(array, func, state, 1);

    if (r.length === 0) return null;
    else return r[0];
}

ArrayHelper.binarySearch = function(array, params) {
...
EventHelper.raiseEvent = function(thisObj, evtName, args) {
    if (thisObj && evtName) {
...
EventHelper.addDomEventHandler = function(element, type, handler) {
...
EventHelper.removeDomEventHandler = function(element, type, handler) {
...
EventHelper.getDomEvent = function() {
...
EventHelper.getSrcElement = function() {
    var evt = EventHelper.getDomEvent();
...
BrowserHelper.get_browserInfo = function() {
    var info = BrowserHelper._browserInfo;
    if (!info) {
        ...
        BrowserHelper._browserInfo = info;
    }

    return info;
}

BrowserHelper.setCssClass = function(element, cls, action) {
    ...
    if (element) {
        ...
        if (action === "add" || action === "remove") {
            var tmp = BrowserHelper.getCssClass(element);
            if (tmp) {
                ...
                for (var i = 0; i < array.length; i++) {
                    str1 = array[i];
                    flag = true;
                    if ((p = ArrayHelper.indexOf(array2, str1)) >= 0) {
                        if (action === "remove") {
                            //flag = false;
                            array.splice(i, 1);
                            i--;
...
BrowserHelper.getCssClass = function(element) {
...
BrowserHelper.setPlainText = function(element, text) {
...
BrowserHelper._getParentElement = function(elt) {
...
BrowserHelper.getParentElement = function(elt, tagName) {
    /// <summary>获取指定元素的父元素。</summary>

    var p = BrowserHelper._getParentElement(elt);

    if (typeof tagName === "string" && tagName.length) {
        while (p && p.tagName !== tagName)
            p = BrowserHelper._getParentElement(p);
    }

    return p;
}

BrowserHelper.isParentOf = function(child, parent) {
    var p = child;
    while (p = BrowserHelper._getParentElement(p)) {
        if (p === parent)
            return true;
    }
...
BrowserHelper.setHtml = function(element, html) {
...
BrowserHelper.translationLocation = function(location, srcElement, targetElement) {

    if (!targetElement)
        ErrorHelper.invalidArgument("targetElement");
    ...
    //var test = BrowserHelper.getLocation(
}

BrowserHelper.getLocation = function(element, relative) {
    if (element) {
        var loc;
...
BrowserHelper.getBounds = function(element, relative) {
    if (element) {
        var bounds = {
        ...
        if (relative) {
            var loc = BrowserHelper.getLocation(element, relative);
            bounds.left = loc.x;
            bounds.top = loc.y;
        }
...
BrowserHelper.getViewPortBounds = function() {
...
BrowserHelper.setBounds = function(element, bounds) {
    if (element) {
...
BrowserHelper.getCurrentStyle = function(element, name) {
    if (element) {
        ...
        if (typeof style !== "undefined") {
            ...
            ErrorHelper.notImplemented();

        return style;
    }
}

BrowserHelper.canElementHaveChildren = function(element) {
    if (element) {
...
Util.getNonClientSize = function(element) {
...
Util.disableTextSelection = function(element) {
    if (element) {
        if (window.event) {
            // IE
...
Util.createHighlightingHtml = function(text, keyword, className) {
    if (!text) return "";
...
Util.createFunctionProxy = function(thisObj, func) {
...
Util.getProperty = function(obj, propName) {
    var name;
...
Util.setProperty = function(obj, propName, value) {
    var name;
...
Util.parseTemplate = function(template, dataSoruce) {
...
Util.containsReference = function(url, tagName, type, attrName, doc) {
    if (!doc) doc = document;
...
Util.addExternalStyleSheet = function(href, position, doc) {
    if (!doc) doc = document;
    if (!Util.containsReference(href, "LINK", "text/css", "href", doc)) {
...
Util.addExternalScript = function(src, doc) {
    if (!doc) doc = document;
    if (!Util.containsReference(src, "SCRIPT", "text/javascript", "src", doc)) {
...
Util.createElement = function(parent, tagName, cssClass, inputType) {
    var elt = parent.ownerDocument.createElement(tagName);
    if (tagName.toUpperCase() === "INPUT") elt.type = inputType;
    parent.appendChild(elt);
    if (cssClass) BrowserHelper.setCssClass(elt, cssClass);
    return elt;
}

DateTimeHelper = {};

DateTimeHelper.weekDayEnum = { Sunday: 0, Monday: 1, Tuesday: 2, Wednesday: 3, Thursday: 4, Friday: 5, Saturday: 6 };
DateTimeHelper.firstDayOfWeek = DateTimeHelper.weekDayEnum.Monday;
DateTimeHelper.shortWeekDayNames = ["日", "一", "二", "三", "四", "五", "六"];
DateTimeHelper.longWeekDayNames = ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"];
DateTimeHelper.shortMonthNames = ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"];
DateTimeHelper.longMonthNames = ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"];
...
DateTimeHelper.getWeekDayName = function(date, shortCase) {
    var day;
    ...
    if (typeof day === "number") {
        ...
        array = shortCase ? DateTimeHelper.shortWeekDayNames : DateTimeHelper.longWeekDayNames;

        return array[day];
    }
    else
        ErrorHelper.invalidArgument("date");
}

DateTimeHelper.getMonthName = function(date, shortCase) {
    var month;
    ...
    if (typeof month === "number") {
        ...
        array = shortCase ? DateTimeHelper.shortMonthNames : DateTimeHelper.longMonthNames;
...
DateTimeHelper.getThisWeek = function(date) {
    ...
    day -= date.getDay() - DateTimeHelper.firstDayOfWeek;
...
DateTimeHelper.getThisMonth = function(date) {
    if (arguments.length > 0 && date.constructor !== Date) {
        ErrorHelper.invalidArgument("date");
    }
    else if (arguments.length === 0)
    ...
    var end = new Date(year, month, DateTimeHelper.getDaysOfMonth(year, month), 23, 59, 59, 999);

    return { start: start, end: end };
    
}

DateTimeHelper.isLeapYear = function(year) {
    if (isNaN(year) || Math.floor(year) !== year)
        ErrorHelper.invalidArgument("year");
...
DateTimeHelper.getDaysOfMonth = function(year, month) {
    if (isNaN(year) || Math.floor(year) !== year)
        ErrorHelper.invalidArgument("year");
    if (isNaN(month) || month < 0 || month > 11 || Math.floor(month) !== month)
        ErrorHelper.invalidArgument("month");

    if (month === 1)
        return DateTimeHelper.isLeapYear(year) ? 29 : 28;
    else if (month < 7)
        return 31 - (month % 2);
    else
        return 30 + (month % 2);
...
DateTimeHelper.parseDateTime = function(text) {
    ...
    if (!isNaN(year)) {
        month--;
        if (year < 1 || year > 9999 || month < 0 || month > 11 || day < 1 || day > DateTimeHelper.getDaysOfMonth(year, month))
            return NaN;
...
// Begin of MSAjaxHelper
MSAjaxHelper = { installed: typeof Sys !== "undefined" };

MSAjaxHelper.registerClass = function(name) {
    if (typeof name === "string" && name.length > 0 && MSAjaxHelper.installed) {
        var type;
...
StringHelper.padLeft = function(text, width, padChar) {
...
Debugging.writeLine = function(format, arg1, arg2) {

    var console = Debugging._getConsole();
...
Debugging._getConsole = function() {
    var console = Debugging._console;
    ...
    if (!console) {
        ...
        Debugging._console = console = div;
        ...
        btn.style.cursor = "pointer";
        console._toolbar.appendChild(btn);
        btn.onclick = Debugging.clearConsole;
        ...
        btn.style.margin = "0px 3px";
        console._toolbar.appendChild(btn);
        btn.onclick = function() { Debugging.hideConsole(); }
...
Debugging.showConsole = function() {
    Debugging._getConsole().style.display = "";
}

Debugging.hideConsole = function() {
    var console = Debugging._console;
...
Debugging.clearConsole = function() {
    var console = Debugging._console;
    if (console) console._output.innerHTML = "";
}

$css = BrowserHelper.setCssClass;
$text = BrowserHelper.setPlainText;
$elt = Util.createElement;
$rect = BrowserHelper.getBounds;

ensureInitialized = function() {
    if (typeof _designMode === "undefined") {
        ...
        for (var i = 0; i < coll.length; i++) {
            ...
            src = obj.src;
            if (src) {
                p = src.indexOf("common.js");
                if (p < 0) p = src.indexOf("js");
                if (p >= 0) {
                    scriptPath = resourcePath = src.substr(0, p);
                    break;
                }
            }
        }
...
getResourcePath = function(filename) {
    if (ensureInitialized()) {
        ...
        if (!path || typeof path !== "string") ErrorHelper.invalidOperation("Critical parameter is not defined.");
...
_registerReference = function(filename, typeName) {
    if (ensureInitialized()) {
        ...
        var url = getResourcePath(filename);
        Util.addExternalScript(url);
    }
}

_registerStyleSheet = function(filename) {
    if (ensureInitialized()) {
        var url = getResourcePath(filename);
        Util.addExternalStyleSheet(url, "first");
    }
}

_createObject = function(typeName, eltid, args) {
    var elt, code;
...
KeyHelper = {};
KeyHelper.translateKeyCode = function(keyCode) {
