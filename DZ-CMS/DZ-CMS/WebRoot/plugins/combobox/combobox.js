...
function ComboBox(element, settings,picturepath) {

    if (!settings) settings = {};
    this._provider = ComboBox._createProvider(settings.style);
    var tmp = this._provider.initialize(this, settings, element,picturepath);
...
ComboBox.registerProvider = function(typeName, style) {
    if (typeof typeName !== "string" || !typeName || typeof window[typeName] !== "function")
        ErrorHelper.invalidArgument("typeName");
    if (!style) ErrorHelper.invalidArgument("style");
    if (typeof style === "string") style = [style];
    else if (!(style instanceof Array)) ErrorHelper.invalidArgument("style");
    ...
    ArrayHelper.forEach(style, function(s) { array[s] = typeName; });
}

ComboBox._createProvider = function(style) {

    if (typeof style !== "string" && style)
        ErrorHelper.invalidArgument("style");

    if (!style) return new _DefaultDropDownProvider();
    var prd = ComboBox._providers[style];
    if (!prd) ErrorHelper.invalidArgument("style");
...
ComboBox.prototype._initialize = function(elt, args) {
    ...
    p = BrowserHelper.getParentElement(elt);
    p.insertBefore(this._element, elt);

    tmp = BrowserHelper.getCurrentStyle(elt, "width");
    ...
    if (this._provider.editable) {
        ...
        handler = Util.createFunctionProxy(this, this._onTextBoxChange);
        EventHelper.addDomEventHandler(input, "paste", handler);
    ...
    if (args.checkbox) {
        ...
        tmp.tabIndex = -1;
        EventHelper.addDomEventHandler(tmp, "click", Util.createFunctionProxy(this, this._onCheckBoxClick));
        this._checkbox = tmp;
    }
    
    this._dropDown = this._createDropDownWindow();

    Util.disableTextSelection(this._dropDown);

    tmp = Util.createFunctionProxy(this, this._onKeyEvent);
    EventHelper.addDomEventHandler(this._element, "keyup", tmp);
    EventHelper.addDomEventHandler(this._element, "keydown", tmp);

    tmp = Util.createFunctionProxy(this, this._onMouseEvent);
    EventHelper.addDomEventHandler(this._element, "click", tmp);
    EventHelper.addDomEventHandler(this._element, "mouseover", tmp);
    EventHelper.addDomEventHandler(this._element, "mouseout", tmp);

    tmp = Util.createFunctionProxy(this, this._onScrollEvent);
    EventHelper.addDomEventHandler(window, "scroll", tmp);
    EventHelper.addDomEventHandler(window, "resize", tmp);
...
ComboBox.prototype._onMouseEvent = function() {
    ...
    if (evt.type === "click") {
        ...
        if (this._button && (src === this._button || BrowserHelper.isParentOf(src, this._button)))
            part = "Button";
        else if (src === this._textBox) {
            part = "TextBox";
        }
        else if (src === this._textCell || BrowserHelper.isParentOf(src, this._textCell)) {
            part = "TextCell";
        }
        ...
        if (src === this._button || BrowserHelper.isParentOf(src, this._button))
...
ComboBox.prototype._raiseValueChangedEvent = function(noCheckBox) {
    ...
    EventHelper.raiseEvent(this, "onValueChanged", { value: value});
}
...
ComboBox.prototype._createDropDownWindow = function() {
    ...
    elt.__babu_nc = Util.getNonClientSize(elt);
...
ComboBox.prototype._onDocumentEvent = function() {
    if (this.isDropDownVisible()) {
        ...
        if (evt.type === "click") {
            if (BrowserHelper.isParentOf(src, this._dropDown))
                close = false;
        }
...
ComboBox.prototype._maintainDropDownPosition = function() {
    ...
    var vp = BrowserHelper.getViewPortBounds();
    ...
    BrowserHelper.setBounds(this._dropDown, pos);
}
...
ComboBox.prototype.openDropDown = function(noFocus) {
    ...
    if (!handler) {
        handler = Util.createFunctionProxy(this, this._onDocumentEvent);
        this._docEventHandler = handler;

        window.setTimeout(function() {
            EventHelper.addDomEventHandler(document, "click", handler);
            EventHelper.addDomEventHandler(window, "blur", handler);
        }, 0);
...
ComboBox.prototype.closeDropDown = function() {
    ...
    if (handler) {
        ...
        EventHelper.removeDomEventHandler(document, "click", handler);
        EventHelper.removeDomEventHandler(window, "blur", handler);
    }
}
...
_DefaultDropDownProvider.prototype.set_itemSource = function(value) {
    if (!value || typeof value === "function" || value instanceof Array) {
        if (typeof value === "function" && this._autoComplete === "none") {
            ErrorHelper.invalidOperation("Dynamic item source should be used only in auto-complete mode.");
        }
        ...
        ErrorHelper.invalidArgument("value");
    }
}
...
_DefaultDropDownProvider.prototype.initialize = function(owner, args, elt,picturepath) {
    var obj = args.buttonImage, tag = elt.tagName;

    if ((tag !== "INPUT" || elt.type !== "text") && tag !== "SELECT")
        ErrorHelper.invalidArgument("element");

    this.buttonImage = getResourcePath(picturepath+"/dda.gif");
    this.buttonDisabledImage = getResourcePath(picturepath+"/dda.gif");
    this._owner = owner;
        ...
        switch (args.autoComplete) {
            case "remote":
                if (typeof args.itemSource !== "function")
                    ErrorHelper.invalidOperation("The itemSource argument must set to a function for remote auto-complete mode.");
            case "local":
    ...
    if (tag === "SELECT") {
        var tmp;
        obj = []
        ArrayHelper.forEach(elt.options, function(opt) {
    ...
    if (args.itemSource) {
        ...
        if (this._autoComplete === "remote")
            ErrorHelper.invalidArgument("itemSource");
...
_DefaultDropDownProvider.prototype._getItemsToRender = function(noRemoteCall) {
    ...
    if (this._itemSource) {
        ...
        if (this._itemSource instanceof Array) {
            ...
            if (key && ac !== "none") {
                items = ArrayHelper.where(items, function(obj) { return obj.text && obj.text.indexOf(key) >= 0 });
            }
        }
        else {
            if (ac === "local")
...
_DefaultDropDownProvider.prototype.renderDropDown = function(element, ownerRect) {
    ...
    if (!list) {
        ...
        list.__babu_loading.src = getResourcePath("loading1.gif");
        list.__babu_loading.display = "none";

        var handler = Util.createFunctionProxy(this, this._mouseEventHandler);

        EventHelper.addDomEventHandler(list, "click", handler);
        EventHelper.addDomEventHandler(list, "mouseover", handler);
        EventHelper.addDomEventHandler(list, "mouseout", handler);
    ...
    if (items) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            text = item.text;
            if (key)
                elt.innerHTML = Util.createHighlightingHtml(text, key, "Keyword");
            else
            ...
            BrowserHelper.setBounds(li, undefined, top);
            top += li.offsetHeight;
        }
        ...
        BrowserHelper.setBounds(list, { width: width, height: height });
    ...
    var updataPos;
    if (!ownerRect) {
        ownerRect = BrowserHelper.getBounds(this._owner.get_element());
        updataPos = true;
    }
    ...
    BrowserHelper.setBounds(element, undefined, undefined, width, height);
...
_DefaultDropDownProvider.prototype._isItemSelected = function(item) {
    if (this._selectedItems) {
        return ArrayHelper.indexOf(this._selectedItems, item) >= 0;
    }
...
_DefaultDropDownProvider.prototype._mouseEventHandler = function() {
    ...
    if (tr.tagName !== "DIV") tr = BrowserHelper.getParentElement(evt.srcElement, "DIV");
    ...
    if (tr.__babu_item) {
        switch (evt.type) {
            case "mouseover":
                ...
                break;
            case "mouseout":
                //if (!BrowserHelper.isParentOf(tr, this._itemTable)) {
    ...
    if (array) {
        ...
        if (typeof value !== "undefined" && value !== null && value !== "") {
            ...
            var func = function(item) {
                ...
                return ArrayHelper.where(value, compare, v, 1).length > 0;
            };
            items = ArrayHelper.where(array, func);
        }
        else items = [];
...
_DefaultDropDownProvider.prototype.get_selectedItems = function() {
    this._ensureSelectedItems();
    if (this._selectedItems) {
        return ArrayHelper.clone(this._selectedItems);
    }
    else
        return [];
}
...
_DefaultDropDownProvider.prototype._selectSibling = function(dir) {
    if (this._itemTable) {
        ...
        if (!tr && length) {
            ...
            index = ArrayHelper.indexOf(tr.parentNode.childNodes, tr) + dir;
...
_DefaultDropDownProvider.prototype.onKeyDown = function(keyCode) {
    //var name = KeyHelper.translateKeyCode(keyCode);
    //Debugging.writeLine(name);
    switch (keyCode) {
        case 9:     // TAB
        case 27:    // ESC
        case 13:    // Enter
...
_registerStyleSheet("combobox.css");
//_registerReference("combobox.datetime.js");
