(function ( $ ) {
	...
	$(document).on('click','.toggler', function(){
		...
		var listaClassi;
		if (container[0].classList){
		  	listaClassi = container[0].classList;
		}else{
			listaClassi = container[0].className.split(" ");
		}
...
function doAnimation(container, containerWidth, sidebarSide, sidebarStatus){
	...
	if(sidebarStatus=="opened"){
		if(sidebarSide=="left"){
			...
			$("#rightblankcol").css("display","none");
			$("#modelcol").removeClass();
			if($("#leftblankcol").is(":hidden")){
				$("#modelcol").addClass("col-xs-12 col-sm-12");
			}else{
		...
		if(sidebarSide=="left"){
			...
			toggleArrow(toggler, "right");
			$("#leftblankcol").css("display","block");
			$("#modelcol").removeClass();
			if($("#rightblankcol").is(":hidden")){
				$("#modelcol").addClass("col-xs-12 col-sm-9");
			}else{
				$("#modelcol").addClass("col-xs-12 col-sm-7");
			}
		}else if(sidebarSide=="right"){
			container.animate({
				right:0
			...
			toggleArrow(toggler, "left");
			$("#rightblankcol").css("display","block");
			$("#modelcol").removeClass();
			if($("#leftblankcol").is(":hidden")){
				$("#modelcol").addClass("col-xs-12 col-sm-10");
			}else{
