...
var initgrid = function(_bizname) {
	...
	if($("#"+_bizname+"_grid").data("gridOptions").exportexcellocal||$("#"+_bizname+"_grid").data("gridOptions").exportexcellocal==undefined){
		$("#"+_bizname+"_grid").navButtonAdd(grid_pager,{
			...
			position:"last",
			title : "export current display data ",
			cursor : "pointer"
		});
	}
	...
	function _exportExcelLocal(_bizname) {
		...
		if (!confirm("Confirm the export current table data for the excel file download?")) {
			return;
		}
		var e = new Array();
		e = $(f).getDataIDs();
...
function getProcessIcons(_bizname,rownum){
	update = "<div class='ui-pg-div ui-inline-edit' style='float: left; margin-left: 8px;' title='modify' onclick=_doopenmodify('"+_bizname+"','"+ rownum
	+ "')><span class='ui-icon ui-icon-pencil'/></div>";
	del = "<div class='ui-pg-div ui-inline-del' style='float: left; margin-left: 5px;' title='delete' onclick=_doopendelete('"+_bizname+"','"+ rownum
		+ "')><span class='ui-icon ui-icon-trash'/></div>";	
	return update + del;
}
//打开修改窗口
...
function _doopendelete(_bizname,selectedRowId) {
	setTimeout(function(){
		parent.bootbox.confirm("Delete the selected record ?", function(result) {
...
function _doeditdelete(_bizname,selectedRowId) {
	setTimeout(function(){
		parent.bootbox.confirm("Delete it ?", function(result) {
			if(result) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				if(dataJson.success) {
					...
					parent.alertMsg("Update success !");
					//修改保存完成
					$("#"+_bizname+"_grid").trigger('domi.modifysave.after');
				} else {
					parent.alertErrorMsg("operation failed:"+dataJson.errorMessage);
...
function checkData(_bizname){
	var selectedRowId = $("#"+_bizname+"_grid").jqGrid("getGridParam", "selrow");
	if (!selectedRowId) {
		alertMsg("Please select the operation line !");
		return false;
	} else {
		return true;
	}
...
var _openDialog4Adding = function(_bizname) {
	...
	parent.createwindow(_bizname,url,"add",_formw,_formh,true);
};
var _openDialog4Modifying = function(_bizname) {
	var flag = checkData(_bizname);
	if(!flag)return;
	var url = $("#"+_bizname+"_grid").data("gridOptions").viewurl;
	parent.createwindow(_bizname,url+getParams(_bizname)+"&actiontype=modify","modify",_formw,_formh,true);
};
...
var _openDialog4Reading = function(_bizname) {
	...
	if(!flag)return;
	var url = $("#"+_bizname+"_grid").data("gridOptions").viewurl;
	parent.createwindow(_bizname,url+getParams(_bizname)+"&actiontype=view&load=detail","view",_formw,_formh,true);
};
...
var _openDialog4Deleting = function(_bizname) {
	...
	if(!flag)return;
	var url = $("#"+_bizname+"_grid").data("gridOptions").viewurl;
	parent.createwindow(_bizname,url+getParams(_bizname)+"&actiontype=delete&load=detail","delete",_formw,_formh,true);
};
...
function _doDeleteSave(_bizname) {
	...
	$.ajax({
		...
		cache : false,
		error : function(data) {
			alertErrorMsg("Ajax interactive error");
		},
		success : function(data) {
			var dataJson = eval(data);
			if(dataJson.success) { 
				$("#"+_bizname+"_grid").jqGrid("delRowData", selectedRowId);
				alertMsg("Delete the information successfully !");
			} else {
				alertErrorMsg("Delete failure:"+dataJson.errorMessage); 
			}
		}
	});
}; 
...
//			async : false,
//			error : function(data) {
//				alertMsg("Ajax interactive error");
//			},
//			success : function(data) {
...
//	}
	...
	for(var i=0;i<dataJson.length;i++){
		...
		}else if(dataJson[i].datatype=="SJLX003" || dataJson[i].datatype=="SJLX004"){//数字型
