...
$("#device_grid").data("gridOptions", {
		height : 272,
		url :  rootpath + '/webpage/biz/dm/deviceteam/devicelist.action?bean.teamid='+teamid,
		colNames : [ 
					'设备ID',
					'所属矿山',
					'设备编号',
...
function onSave(){
	...
	try{
		parent.PageObject.itemMap['deviceteam_selectdevice'].callback(data);
	}catch(e){
		parent.closewin('deviceteam_selectdevice');
	}
	parent.closewin('deviceteam_selectdevice');
}
function uncheck() {
	parent.PageObject.itemMap['deviceteam_selectdevice'].callback([{"paramname":"","code":""}]);
	parent.closewin('deviceteam_selectdevice');
}
