...
<script type="text/javascript" src="<%=request.getContextPath()%>/plugins/jquery/botree.js"></script>
		<script type="text/javascript">
			...
			var qdid = "<%=request.getParameter("qdid")%>";
			$(function(){
			...
			function onGetNode() {
				parent.fullShow(granting);
			 	...
			 	$.ajax({
					url: rootpath+'/biz/bi/areaproject/addAreaProject.action',
					data: { 'param' : projectids,'pids':pids, 'areaProject.deptid':qdid },
					cache: false,
			...
			function revokeNode() {
				parent.fullShow(revoking);
			 	...
			 	$.ajax({
					url: rootpath+'/biz/bi/areaproject/delAreaProject.action',
					data: { 'param' : projectids, 'areaProject.deptid':qdid },
					cache: false,
					success: function(data){
						parent.fullHide();
						var dataJson = eval(data);
