	...
	function getTreeNodes_weishouyu(node) {
		...
		$.ajax({
			url: rootpath+'/manager/sys/rolepopedom/rolepopedomunlist.action',
			...
			success: function(data){
				...
				if(dataJson.success){
					...
					for(var i = 0; i < length; i++){
						...
						addNode_weishouyu(obj.id, obj.name, obj.pid);
					}
				}else{
	...
	function addNode_weishouyu(nodeID, nodeName) {
		if(!nodeID){
			nodeID = '';
		}
		if(!nodeName){
		...
		BOTree_weishouyu.addNodes(srcNode[0], newNode);
	}
