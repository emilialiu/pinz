...
var options={pid:pid,pdid:pdid}
$(function(){
	//var pid=$(options).attr("pid");
	//var pdid = $(options).attr("pdid");
	alert(parent);
	alert($("iframe").html());
	alert($(parent).find("#act_processinstanceview").html());
	var _defaults = {
	        srcEle: this,
	        pid: pid,
		    pdid: pdid
	 ...
	 // 获取图片资源
	 var imageUrl = rootpath + "/manager/activity/view.action?bean.id=" +opts.pdid+ "&resourceType=image";
	 $.ajax({  
	        url : rootpath + '/manager/activity/trace.action?bean.id='+opts.pid,
	        data :'',  
	        cache : false,  
	        async:false, 
	        error : function(data) {  
 	...
 	$('#diagram-viewer').on('click', function() {
 		...
 		var url = rootpath + '/activiti/diagram-viewer/index.html?processDefinitionId=' + opts.pdid + '&processInstanceId=' + opts.pid;
 	...
 	//});
      ...
      /*  $('#workflowTraceDialog').dialog({
            ...
            open: function() {
                ...
                $('#workflowTraceDialog').dialog('option', 'title', '查看流程（按ESC键可以关闭）<button style="background-color:blue" id="diagram-viewer">Diagram-Viewer</button>');
