				...
				function() {
					$(function() {
						...
						$("#act_task_dyform_info").dialog({
							...
							buttons : { // 为对话框添加按钮
								"取消" : function() {
									$("#act_task_dyform_info").dialog("close");
								},
								"提交" : function() {
									subdata();
								}
	 ...
	 $.ajax({  
	        ...
	        success : function(data) {  
	            ...
	            if(dataJson.success) {
	                ...
	                $("#act_task_grid").trigger("reloadGrid");
	            } else {  
...
function genform(id,name){
    ...
    $('#act_task_dyform_form').html(trs);
    $("#act_task_dyform_info").dialog("option", "title","处理"+name).dialog("open");
 ...
 */
function subdata(){
	var consng=$('#act_task_dyform_form');
