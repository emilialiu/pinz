	...
	$("#dz_home_page_manager_grid").data("gridOptions", {
		...
		colModel : [ //	align:left,center,right,默认left;classes:列的css;datefmt:默认Y-m-d;defval:查询字段的默认值;editable:单元格是否可编辑,默认false;
					...
					{name : 'processbtn',index:'', width:100, fixed:true, sortable:false, resize:false,search : false,formatter:practice_btnformatter},
						...
						{name : 'img_url',index : 'img_url',fixed : true,width : 500,sorttype : 'varchar(512)',editable : false,searchoptions : {sopt : [ 'cn', 'eq', 'ne', 'le', 'lt', 'gt', 'ge' ]}},
						{name : 'isstart',index : 'isstart',fixed : true,width : 100,sorttype : 'int(11)',editable : false,searchoptions : {sopt : [ 'cn', 'eq', 'ne', 'le', 'lt', 'gt', 'ge' ]}},
...
function practice_btnformatter(cellvalue,options,rowobj){
	update = "<div class='ui-pg-div' style='float: left; margin-left: 5px;' title='修改' onclick=_doopenmodify('dz_home_page_manager','"+options.rowId+"')><span class='ui-icon ui-icon-pencil'/></div>";
	delet = "<div class='ui-pg-div' style='float: left; margin-left: 5px;' title='删除' onclick=_doopendelete('dz_home_page_manager','"+options.rowId+"')><span class='ui-icon ui-icon-trash'/></div>";
	detail = "<div class='ui-pg-div' style='float: left; margin-left: 5px;' title='查看图片' onclick=openfile('dz_home_page_manager','"+options.rowId+"')><span class='ui-icon fa-eye'/></div>";
	return update+delet+detail;
}
function openfile(_bizname,rowid){
	var selectedRowId = $("#"+_bizname+"_grid").jqGrid("getGridParam", "selrow"); 
	var rowData =$("#"+_bizname+"_grid").jqGrid('getRowData', selectedRowId);
	window.open(rootpath + "/webpage/biz/showimg.jsp?path="+rowData.img_url);
}
