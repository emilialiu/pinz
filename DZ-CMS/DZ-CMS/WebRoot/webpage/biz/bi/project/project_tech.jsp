		...
		<div class="col-xs-12 col-sm-2" style="padding-right:4px;">
			<div class="widget-box widget-color-blue">
				<div class="widget-header">
					<h5 class="widget-title"><i class="ace-icon fa fa-sitemap"></i><s:text name="dmmes.bi.projectinfo.technologytree" /></h5>
				</div>
				<div>
					<ul id="technology_tree" class="ztree" style="height: 861px;overflow:auto;"></ul>
				</div>
			</div>
		</div>
...
<script type="text/javascript" src="<%=request.getContextPath()%>/webpage/biz/bi/project/techtree.js"></script>
<script type="text/javascript" src="<%=request.getContextPath()%>/webpage/biz/bi/project/pubbiz.js"></script>
<script>
var chosetechnology='<s:text name="dmmes.bi.chosetechnology" />';
var projecttotechnology='<s:text name="dmmes.bi.projecttotechnology" />';
...
$(".addproject").click(function(){
	...
	if(!srcNode[0].isChild){
		alertMsg(chosetechnology);
		return;
	}
	parent.openwin_nobutton("project","/webpage/pub/select/project_tech/project.jsp?techid="+srcNode[0].id,projecttotechnology,850,600);
})
...
function getNodeInfo(techid){
	if(tabindex == 0){
		$("#roadway_grid").setGridParam({page:1,postData:{"bean.techid": techid}}).trigger("reloadGrid");
		$("#projectsupport_grid").jqGrid('setGridParam',{postData:{'bean.projectid':''}}).trigger("reloadGrid");
		$("#roadway_installinfo_grid").jqGrid('setGridWidth', $("#roadway_installinfo_grid_box").width());
	}else if(tabindex == 1){
		$("#stope_grid").setGridParam({page:1,postData:{"bean.techid": techid}}).trigger("reloadGrid");
		...
		$("#other_grid").setGridParam({page:1,postData:{"bean.techid": techid}}).trigger("reloadGrid");
	}
}
//打开删除窗口
