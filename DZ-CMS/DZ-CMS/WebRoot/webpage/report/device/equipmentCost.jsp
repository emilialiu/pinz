...
<div class="row">
	<div class="col-xs-12">
		<form id="reportforms_equipmentCost" action="<%=request.getContextPath()%>/pub/report/doReport.action" target="reportframe" >
			<input type="hidden" name="reportclass" value="equipment_cost"/>
 			<input type="hidden" name="reportformat" value="html"/>
			<input type="hidden" name="reporttitle" value="设备成本分析"/>
...
function doSearch(){
	...
	$("#reportforms_equipmentCost").find("#startDate").val(startDate);
	$("#reportforms_equipmentCost").find("#endDate").val(endDate);
	$("#reportforms_equipmentCost")[0].submit();
}	
</script>
</body>
</html>