...
import com.dimine.home.service.Dz_home_page_managerService;

/**
 * 用于对首页banner管理进行系列的操作的action
 * 
 * @author dimine 2019-08-29 20:03:20
 * 
 */
@Namespace("/webpage/biz/home/page")
@Scope("request")
@Controller("dz_home_page_managerAction")
public class Dz_home_page_managerAction extends BaseAction {
	private static final long serialVersionUID = 1L;
	private final static Logger logger = Logger.getLogger(Dz_home_page_managerAction.class);
	// 业务处理
	@Resource
	private Dz_home_page_managerService<Dz_home_page_managerEntity> dz_home_page_managerService;

	// 参数实体
	private Dz_home_page_managerEntity bean = new Dz_home_page_managerEntity();
	/**
	 * 添加类型 addmore为新增保存
	 */
	...
	public String list() {
		try {
			...
			List<Dz_home_page_managerEntity> dataList = dz_home_page_managerService.selectByList(bean);
			// 设置页面数据
	...
	/**
	 * 新增首页banner管理信息查询 <功能详细描述>
	 * 
	 * @return
	 * @see [类、类#方法、类#成员]
	 */
	@Action(value = "doAdd", results = { @Result(name = "success", location = "/webpage/biz/home/page/info.jsp") })
	public String doAdd() {
		// TODO 其他业务处理
	...
	public String doAddSave() {
		try {
			dz_home_page_managerService.insert(bean);
	...
	public String doDelete() {
		try {
			dz_home_page_managerService.delete(bean);
		} catch (Exception e) {
			bean.setErrorMessage("failed");
			e.printStackTrace();
		}
	 ...
	 * 对首页banner管理进行修改查询操作
	 */
	@Action(value = "doModify", results = { @Result(name = "success", location = "/webpage/biz/home/page/info.jsp") })
	public String doModify() {
		try {
			...
			bean = dz_home_page_managerService.selectById(bean.getId());
		} catch (Exception e) {
	...
	public Dz_home_page_managerEntity getBean() {
		return bean;
	}

	public void setBean(Dz_home_page_managerEntity bean) {
		this.bean = bean;
	}

	public Dz_home_page_managerService<Dz_home_page_managerEntity> getDz_home_page_managerService() {
		return dz_home_page_managerService;
	}

	public void setDz_home_page_managerService(Dz_home_page_managerService<Dz_home_page_managerEntity> dz_home_page_managerService) {
		this.dz_home_page_managerService = dz_home_page_managerService;
	}
