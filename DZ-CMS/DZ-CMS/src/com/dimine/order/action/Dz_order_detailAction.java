package com.dimine.order.action;
...
import com.dimine.order.service.Dz_order_detailService;

/**
 * 用于对商品明细进行系列的操作的action
 * 
 * @author dimine 2019-09-03 19:10:58
 * 
 */
@Namespace("/webpage/biz/order/detail")
@Scope("request")
@Controller("dz_order_detailAction")
public class Dz_order_detailAction extends BaseAction {
	private static final long serialVersionUID = 1L;
	private final static Logger logger = Logger.getLogger(Dz_order_detailAction.class);
	// 业务处理
	@Resource
	private Dz_order_detailService<Dz_order_detailEntity> dz_order_detailService;

	// 参数实体
	private Dz_order_detailEntity bean = new Dz_order_detailEntity();
	/**
	 * 添加类型 addmore为新增保存
	 */
	...
	public String list() {
		try {
			...
			List<Dz_order_detailEntity> dataList = dz_order_detailService.selectByList(bean);
			// 设置页面数据
	 ...
	 * @see [类、类#方法、类#成员]
	 */
	@Action(value = "doAdd", results = { @Result(name = "success", location = "/webpage/biz/order/detail/info.jsp") })
	public String doAdd() {
		// TODO 其他业务处理
	...
	public String doAddSave() {
		try {
			dz_order_detailService.insert(bean);
	...
	public String doDelete() {
		try {
			dz_order_detailService.delete(bean);
		} catch (Exception e) {
			bean.setErrorMessage("failed");
			e.printStackTrace();
		}
	 ...
	 * 对商品明细进行修改查询操作
	 */
	@Action(value = "doModify", results = { @Result(name = "success", location = "/webpage/biz/order/detail/info.jsp") })
	public String doModify() {
		try {
			...
			bean = dz_order_detailService.selectById(bean.getId());
		} catch (Exception e) {
	...
	public String doModifySave() {
		try {
   			dz_order_detailService.update(bean, getActiontype());				
		} catch (ValidException ee) {
			bean.setErrorMessage(ee.getMessage());
			ee.printStackTrace();
		} catch (Exception e) {
	...
	public Dz_order_detailEntity getBean() {
		return bean;
	}

	public void setBean(Dz_order_detailEntity bean) {
		this.bean = bean;
	}

	public Dz_order_detailService<Dz_order_detailEntity> getDz_order_detailService() {
		return dz_order_detailService;
	}

	public void setDz_order_detailService(Dz_order_detailService<Dz_order_detailEntity> dz_order_detailService) {
		this.dz_order_detailService = dz_order_detailService;
	}
