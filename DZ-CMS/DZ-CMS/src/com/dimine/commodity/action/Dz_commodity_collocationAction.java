	...
	public String list() {
		try {
			...
			for(int j = 0;j<dataList.size();j++){
				if(dataList.get(j).getSingle_ids().indexOf(",")>0){
					String[] ids = dataList.get(j).getSingle_ids().split(",");
					String names = "";
					for(int i=0;i<ids.length;i++){
						names+=dz_commodity_collocationService.findsp(ids[i]);
						if(i!=ids.length-1){
							names+=",";
						}
					}
					dataList.get(j).setNames(names);
				}
