package com.dimine.bi.action;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

import javax.annotation.Resource;

import org.apache.log4j.Logger;
import org.apache.struts2.convention.annotation.Action;
import org.apache.struts2.convention.annotation.Namespace;
import org.apache.struts2.convention.annotation.Result;
import org.springframework.context.annotation.Scope;
import org.springframework.stereotype.Controller;

import com.dimine.base.action.BaseAction;
import com.dimine.base.common.ValidException;
import com.dimine.base.util.SQLCallback;
import com.dimine.base.util.SQLTool;
import com.dimine.base.util.json.JSONUtil;
import com.dimine.bi.entity.T_bi_mineEntity;
import com.dimine.bi.service.T_bi_mineService;
import com.dimine.sys.entity.DeptEntity;
import com.dimine.sys.service.DeptService;

/**
 * 用于对矿山信息(T_BI_Mine)进行系列的操作的action
 * 
 * @author dimine 2017-08-15 10:30:26
 * 
 */
@Namespace("/webpage/biz/bi/mine")
@Scope("request")
@Controller("t_bi_mineAction")
public class T_bi_mineAction extends BaseAction {
	private static final long serialVersionUID = 1L;
	private final static Logger logger = Logger
			.getLogger(T_bi_mineAction.class);
	// 业务处理
	@Resource
	private T_bi_mineService<T_bi_mineEntity> t_bi_mineService;
	@Resource
	private DeptService<DeptEntity> deptService;

	// 参数实体
	private T_bi_mineEntity bean = new T_bi_mineEntity();
	/**
	 * 添加类型 addmore为新增保存
	 */
	private String addtype;

	private String somename;

	private String encode;

	private String param;

	private String filters;

	/**
	 * 对矿山信息(T_BI_Mine)进行列表查询操作
	 * 
	 * @return
	 */
	@Action(value = "list", results = { @Result(name = "success", location = "/webpage/pub/grid/pager.jsp") })
	public String list() {
		try {
			if (filters != null) {
				String query = new SQLTool().constructWhere(filters,
						new SQLCallback() {
							@Override
							public String executeField(String f) {
								if (f.equals("orgname"))
									return "d.deptname";
								if (f.equals("oremanutypename"))
									return "d1.PARAMNAME";
								if (f.equals("miningmethodname"))
									return "d2.PARAMNAME";
								if (f.equals("productivestatusname"))
									return "d3.PARAMNAME";
								if (f.equals("countryname"))
									return "d4.PARAMNAME";
								if (f.equals("provincename"))
									return "d5.PARAMNAME";
								if (f.equals("cityname"))
									return "d6.PARAMNAME";
								return f;
							}

							@Override
							public String executeData(String f, String o,
									String d) {
								if (o.equals("bw") || o.equals("en"))
									return (new StringBuilder("'")).append(d)
											.append("%'").toString();
								if (o.equals("ew") || o.equals("en"))
									return (new StringBuilder("'%")).append(d)
											.append("'").toString();
								if (o.equals("cn") || o.equals("nc"))
									return (new StringBuilder("'%")).append(d)
											.append("%'").toString();
								else
									return (new StringBuilder("'")).append(d)
											.append("'").toString();
							}
						});
				bean.setQuery(query);
			}
			bean.setPager(this.getPager());
			List<T_bi_mineEntity> dataList = t_bi_mineService
					.selectByList(bean);
			// 设置页面数据
			Map<String, Object> jsonMap = new HashMap<String, Object>();
			jsonMap.put("page", bean.getPager().getPageId());
			jsonMap.put("total", bean.getPager().getPageCount());
			jsonMap.put("records", bean.getPager().getRowCount());
			jsonMap.put("rows", dataList);
			this.setJsonStr(JSONUtil.toJSONString(jsonMap));
		} catch (Exception e) {
			this.setErrorMessage(e);
		}
		return SUCCESS;
	}

	/**
	 * 新增矿山信息(T_BI_Mine)信息查询 <功能详细描述>
	 * 
	 * @return
	 * @see [类、类#方法、类#成员]
	 */
	@Action(value = "doAdd", results = { @Result(name = "success", location = "/webpage/biz/bi/mine/info.jsp") })
	public String doAdd() {
		// TODO 其他业务处理
		bean.setOrgid(getLoginUser().getDeptid());
		bean.setOrgname(deptService.getDept(getLoginUser().getDeptid())
				.getDeptname());
		bean.setProductivepower("0");
		bean.setSiftpower("0");
		bean.setArea("0");
		bean.setMinewidth("0");
		bean.setMinelength("0");
		bean.setMineheight("0");
		bean.setStrippingratio("0");
		bean.setExplorationratio("0");
		bean.setWorkusercount("0");
		bean.setTechusercount("0");
		this.setActiontype(ACTIONTYPE_ADDSAVE);
		return SUCCESS;
	}

	/**
	 * 新增矿山信息(T_BI_Mine)信息保存 <功能详细描述>
	 * 
	 * @return
	 * @see [类、类#方法、类#成员]
	 */
	@Action(value = "doAddSave", results = { @Result(name = "success", location = "/webpage/pub/beanresult.jsp") })
	public String doAddSave() {
		try {
			t_bi_mineService.insert(bean);
		} catch (ValidException ee) {
			bean.setErrorMessage(ee.getMessage());
			ee.printStackTrace();
		} catch (Exception e) {
			bean.setErrorMessage("failed");
			e.printStackTrace();
		}
		this.setJsonStr(JSONUtil.toJSONString(bean));
		return SUCCESS;
	}

	/**
	 * 删除矿山信息(T_BI_Mine)信息 <功能详细描述>
	 * 
	 * @return
	 * @see [类、类#方法、类#成员]
	 */
	@Action(value = "delete", results = { @Result(name = "success", location = "/webpage/pub/beanresult.jsp") })
	public String doDelete() {
		try {
			t_bi_mineService.delete(bean);
		} catch (Exception e) {
			bean.setErrorMessage("failed");
			e.printStackTrace();
		}
		this.setJsonStr(JSONUtil.toJSONString(bean));
		return SUCCESS;
	}

	/**
	 * 对矿山信息(T_BI_Mine)进行修改查询操作
	 */
	@Action(value = "doModify", results = { @Result(name = "success", location = "/webpage/biz/bi/mine/info.jsp") })
	public String doModify() {
		try {
			if (getActiontype().equals(ACTIONTYPE_MODIFY)) {
				this.setActiontype(ACTIONTYPE_MODIFYSAVE);
			} else if (getActiontype().equals(ACTIONTYPE_VIEW)) {
				this.setActiontype(ACTIONTYPE_VIEW);
			} else if (getActiontype().equals(ACTIONTYPE_DELETE)) {
				this.setActiontype(ACTIONTYPE_DELETESAVE);
			}
			bean = t_bi_mineService.selectById(bean.getMineid());
		} catch (Exception e) {
			this.setErrorMessage(this.getText("modifyfailure"));
		}
		return SUCCESS;
	}

	/**
	 * 对矿山信息(T_BI_Mine)进行存储和修改操作
	 */
	@Action(value = "doModifySave", results = { @Result(name = "success", location = "/webpage/pub/beanresult.jsp") })
	public String doModifySave() {
		try {
			t_bi_mineService.update(bean, getActiontype());
		} catch (ValidException ee) {
			bean.setErrorMessage(ee.getMessage());
			ee.printStackTrace();
		} catch (Exception e) {
			bean.setErrorMessage("failed");
			e.printStackTrace();
		}
		this.setJsonStr(JSONUtil.toJSONString(bean));
		return SUCCESS;
	}

	// 日志文件用
	@Override
	public String getBiztypename() {
		return "矿山信息(T_BI_Mine)信息管理";
	}

	public T_bi_mineEntity getBean() {
		return bean;
	}

	public void setBean(T_bi_mineEntity bean) {
		this.bean = bean;
	}

	public T_bi_mineService<T_bi_mineEntity> getT_bi_mineService() {
		return t_bi_mineService;
	}

	public void setT_bi_mineService(
			T_bi_mineService<T_bi_mineEntity> t_bi_mineService) {
		this.t_bi_mineService = t_bi_mineService;
	}

	public String getAddtype() {
		return addtype;
	}

	public void setAddtype(String addtype) {
		this.addtype = addtype;
	}

	public String getSomename() {
		return somename;
	}

	public void setSomename(String somename) {
		this.somename = somename;
	}

	public String getEncode() {
		return encode;
	}

	public void setEncode(String encode) {
		this.encode = encode;
	}

	public String getParam() {
		return param;
	}

	public void setParam(String param) {
		this.param = param;
	}

	public String getFilters() {
		return filters;
	}

	public void setFilters(String filters) {
		this.filters = filters;
	}

	public DeptService<DeptEntity> getDeptService() {
		return deptService;
	}

	public void setDeptService(DeptService<DeptEntity> deptService) {
		this.deptService = deptService;
	}

}
