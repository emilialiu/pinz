	...
	function reloadTree(node) {
		var treeData = [];
		$.ajax({
			url: rootpath+'/webpage/biz/sc/process/tree.action',
			cache:false,
			...
			success: function(data){
				...
				if(dataJson.success){
					...
					BOTree1 = $.fn.zTree.init($("#process_tree"), setting, root);
	...
	function getTreeNodes(node) {
		...
		var treeData = [];
		var data;
		var url = rootpath+'/webpage/biz/sc/process/tree.action';
		$.ajax({
			url: url,
			data: {"bean.procid":node.id,"bean.isParent":node.isParent,"bean.level":node.level},
			cache: false,
			success: function(data){
				var dataJson = eval('('+data+')');
				if(dataJson.success){
