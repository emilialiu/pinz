...
//			"maxItems" : 20,
	...
	var b = function(q) {
		var r = this;
		this.inputView = g(q);
		this.inputView.attr("autocomplete", "off").keyup(
				this._keyup = function(s) {
					switch (s.keyCode) {
					case 13:
					case 16:
					case 17:
	...
	var j = function() {
		var q = this;
		this.searchView = g("<div class='ac'><ul></ul></div>").appendTo(
	...
	var i = function(q) {
		...
		if (g.inArray(this.option.listStyle, [ "normal", "iconList",
						"custom" ]) == -1) {
			throw [ "遇到未知的listStyle参数！" ]
		}
		g.each(q, function(u, w) {
			var v = g("<li><div></div></li>").appendTo(r).addClass(
			...
			case "iconList":
				...
				v.append(g("<div></div>").append(t)).append(
						"<span>" + w.label + "</span>");
				break;
			case "custom":
			...
			if (s.option.itemHeight > 0) {
				v.height(s.option.itemHeight).css("max-height",
						s.option.itemHeight)
			}
		})
	};
	var k = function() {
		if (this.option.listDirection === "down") {
			...
			if (this.option.listDirection === "up") {
				var r = this.inputView.offset().top
						- this.searchView.outerHeight()
			} else {
	...
	var d = function() {
		if (typeof (this.option.width) === "string"
				&& this.option.width.toLowerCase() === "auto") {
			...
			if (this.option.maxHeight > 0) {
				...
				if (g.browser.msie) {
					this.searchView
							.css(
									"height",
									this.searchView.height() > this.option.maxHeight ? this.option.maxHeight
											+ "px"
											: "auto")
				}
			}
	...
	var p = function(s) {
		...
		if (t.size()) {
			...
			var q = s === "up" ? this.searchView.find("li").last()
					: this.searchView.find("li").first()
		}
		if (q.size()) {
			...
			if (u + r > this.searchView.height()) {
				this.searchView.scrollTop(this.searchView.scrollTop() + r + u
	...
	f.prototype.destroy = function() {
		...
		this.inputView.unbind("keyup", this._keyup).unbind("keydown",
				this._keydown).unbind("blur", this._blur);
