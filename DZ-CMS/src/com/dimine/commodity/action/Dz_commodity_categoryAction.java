...
import com.dimine.commodity.service.Dz_commodity_categoryService;

/**
 * 用于对商品分类表进行系列的操作的action
 * 
 * @author dimine 2019-08-29 20:06:46
 * 
 */
@Namespace("/webpage/biz/commodity/category")
@Scope("request")
@Controller("dz_commodity_categoryAction")
public class Dz_commodity_categoryAction extends BaseAction {
	private static final long serialVersionUID = 1L;
	private final static Logger logger = Logger.getLogger(Dz_commodity_categoryAction.class);
	// 业务处理
	@Resource
	private Dz_commodity_categoryService<Dz_commodity_categoryEntity> dz_commodity_categoryService;

	// 参数实体
	private Dz_commodity_categoryEntity bean = new Dz_commodity_categoryEntity();
	/**
	 * 添加类型 addmore为新增保存
	 */
	...
	public String list() {
		try {
			...
			List<Dz_commodity_categoryEntity> dataList = dz_commodity_categoryService.selectByList(bean);
			// 设置页面数据
	 ...
	 * @see [类、类#方法、类#成员]
	 */
	@Action(value = "doAdd", results = { @Result(name = "success", location = "/webpage/biz/commodity/category/info.jsp") })
	public String doAdd() {
		// TODO 其他业务处理
	...
	public String doAddSave() {
		try {
			dz_commodity_categoryService.insert(bean);
	...
	public String doDelete() {
		try {
			dz_commodity_categoryService.delete(bean);
		} catch (Exception e) {
			bean.setErrorMessage("failed");
			e.printStackTrace();
		}
	 ...
	 * 对商品分类表进行修改查询操作
	 */
	@Action(value = "doModify", results = { @Result(name = "success", location = "/webpage/biz/commodity/category/info.jsp") })
	public String doModify() {
		try {
			...
			bean = dz_commodity_categoryService.selectById(bean.getId());
		} catch (Exception e) {
	...
	public String doModifySave() {
		try {
   			dz_commodity_categoryService.update(bean, getActiontype());				
		} catch (ValidException ee) {
			bean.setErrorMessage(ee.getMessage());
			ee.printStackTrace();
		} catch (Exception e) {
	...
	public Dz_commodity_categoryEntity getBean() {
		return bean;
	}

	public void setBean(Dz_commodity_categoryEntity bean) {
		this.bean = bean;
	}

	public Dz_commodity_categoryService<Dz_commodity_categoryEntity> getDz_commodity_categoryService() {
		return dz_commodity_categoryService;
	}

	public void setDz_commodity_categoryService(Dz_commodity_categoryService<Dz_commodity_categoryEntity> dz_commodity_categoryService) {
		this.dz_commodity_categoryService = dz_commodity_categoryService;
	}
