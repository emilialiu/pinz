...
import com.dimine.commodity.service.Dz_commodity_reviewService;

/**
 * 用于对商品评价表进行系列的操作的action
 * 
 * @author dimine 2019-08-29 19:55:44
 * 
 */
@Namespace("/webpage/biz/commodity/review")
@Scope("request")
@Controller("dz_commodity_reviewAction")
public class Dz_commodity_reviewAction extends BaseAction {
	private static final long serialVersionUID = 1L;
	private final static Logger logger = Logger.getLogger(Dz_commodity_reviewAction.class);
	// 业务处理
	@Resource
	private Dz_commodity_reviewService<Dz_commodity_reviewEntity> dz_commodity_reviewService;

	// 参数实体
	private Dz_commodity_reviewEntity bean = new Dz_commodity_reviewEntity();
	/**
	 * 添加类型 addmore为新增保存
	 */
	...
	public String list() {
		try {
			...
			List<Dz_commodity_reviewEntity> dataList = dz_commodity_reviewService.selectByList(bean);
			// 设置页面数据
	 ...
	 * @see [类、类#方法、类#成员]
	 */
	@Action(value = "doAdd", results = { @Result(name = "success", location = "/webpage/biz/commodity/review/info.jsp") })
	public String doAdd() {
		// TODO 其他业务处理
	...
	public String doAddSave() {
		try {
			dz_commodity_reviewService.insert(bean);
	...
	public String doDelete() {
		try {
			dz_commodity_reviewService.delete(bean);
		} catch (Exception e) {
			bean.setErrorMessage("failed");
			e.printStackTrace();
		}
	 ...
	 * 对商品评价表进行修改查询操作
	 */
	@Action(value = "doModify", results = { @Result(name = "success", location = "/webpage/biz/commodity/review/info.jsp") })
	public String doModify() {
		try {
			...
			bean = dz_commodity_reviewService.selectById(bean.getId());
		} catch (Exception e) {
	...
	public String doModifySave() {
		try {
   			dz_commodity_reviewService.update(bean, getActiontype());				
		} catch (ValidException ee) {
			bean.setErrorMessage(ee.getMessage());
			ee.printStackTrace();
		} catch (Exception e) {
	...
	public Dz_commodity_reviewEntity getBean() {
		return bean;
	}

	public void setBean(Dz_commodity_reviewEntity bean) {
		this.bean = bean;
	}

	public Dz_commodity_reviewService<Dz_commodity_reviewEntity> getDz_commodity_reviewService() {
		return dz_commodity_reviewService;
	}

	public void setDz_commodity_reviewService(Dz_commodity_reviewService<Dz_commodity_reviewEntity> dz_commodity_reviewService) {
		this.dz_commodity_reviewService = dz_commodity_reviewService;
	}
