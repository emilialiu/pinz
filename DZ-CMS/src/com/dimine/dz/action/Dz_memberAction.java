...
import com.dimine.dz.service.Dz_memberService;

/**
 * 用于对客户等级管理进行系列的操作的action
 * 
 * @author dimine 2019-08-29 19:13:18
 * 
 */
@Namespace("/webpage/biz/dz/member")
@Scope("request")
@Controller("dz_memberAction")
public class Dz_memberAction extends BaseAction {
	private static final long serialVersionUID = 1L;
	private final static Logger logger = Logger.getLogger(Dz_memberAction.class);
	// 业务处理
	@Resource
	private Dz_memberService<Dz_memberEntity> dz_memberService;

	// 参数实体
	private Dz_memberEntity bean = new Dz_memberEntity();
	/**
	 * 添加类型 addmore为新增保存
	 */
	...
	public String list() {
		try {
			...
			List<Dz_memberEntity> dataList = dz_memberService.selectByList(bean);
			// 设置页面数据
	 ...
	 * @see [类、类#方法、类#成员]
	 */
	@Action(value = "doAdd", results = { @Result(name = "success", location = "/webpage/biz/dz/member/info.jsp") })
	public String doAdd() {
		// TODO 其他业务处理
	 ...
	 * 对客户等级管理进行修改查询操作
	 */
	@Action(value = "doModify", results = { @Result(name = "success", location = "/webpage/biz/dz/member/info.jsp") })
	public String doModify() {
		try {
			...
			bean = dz_memberService.selectById(bean.getId());
		} catch (Exception e) {
	...
	public String doModifySave() {
		try {
   			dz_memberService.update(bean, getActiontype());				
		} catch (ValidException ee) {
			bean.setErrorMessage(ee.getMessage());
			ee.printStackTrace();
		} catch (Exception e) {
	...
	public Dz_memberEntity getBean() {
		return bean;
	}

	public void setBean(Dz_memberEntity bean) {
		this.bean = bean;
	}

	public Dz_memberService<Dz_memberEntity> getDz_memberService() {
		return dz_memberService;
	}

	public void setDz_memberService(Dz_memberService<Dz_memberEntity> dz_memberService) {
		this.dz_memberService = dz_memberService;
	}
