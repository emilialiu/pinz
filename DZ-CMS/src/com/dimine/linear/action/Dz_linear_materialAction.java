package com.dimine.linear.action;
...
import com.dimine.linear.service.Dz_linear_materialService;

/**
 * 用于对线性材质管理进行系列的操作的action
 * 
 * @author dimine 2019-08-29 20:09:14
 * 
 */
@Namespace("/webpage/biz/linear/material")
@Scope("request")
@Controller("dz_linear_materialAction")
public class Dz_linear_materialAction extends BaseAction {
	private static final long serialVersionUID = 1L;
	private final static Logger logger = Logger.getLogger(Dz_linear_materialAction.class);
	// 业务处理
	@Resource
	private Dz_linear_materialService<Dz_linear_materialEntity> dz_linear_materialService;

	// 参数实体
	private Dz_linear_materialEntity bean = new Dz_linear_materialEntity();
	/**
	 * 添加类型 addmore为新增保存
	 */
	...
	public String list() {
		try {
			...
			List<Dz_linear_materialEntity> dataList = dz_linear_materialService.selectByList(bean);
			// 设置页面数据
	 ...
	 * @see [类、类#方法、类#成员]
	 */
	@Action(value = "doAdd", results = { @Result(name = "success", location = "/webpage/biz/linear/material/info.jsp") })
	public String doAdd() {
		// TODO 其他业务处理
	...
	public String doAddSave() {
		try {
			dz_linear_materialService.insert(bean);
	...
	public String doDelete() {
		try {
			dz_linear_materialService.delete(bean);
		} catch (Exception e) {
			bean.setErrorMessage("failed");
			e.printStackTrace();
		}
	 ...
	 * 对线性材质管理进行修改查询操作
	 */
	@Action(value = "doModify", results = { @Result(name = "success", location = "/webpage/biz/linear/material/info.jsp") })
	public String doModify() {
		try {
			...
			bean = dz_linear_materialService.selectById(bean.getId());
		} catch (Exception e) {
	...
	public String doModifySave() {
		try {
   			dz_linear_materialService.update(bean, getActiontype());				
		} catch (ValidException ee) {
			bean.setErrorMessage(ee.getMessage());
			ee.printStackTrace();
		} catch (Exception e) {
	...
	public Dz_linear_materialEntity getBean() {
		return bean;
	}

	public void setBean(Dz_linear_materialEntity bean) {
		this.bean = bean;
	}

	public Dz_linear_materialService<Dz_linear_materialEntity> getDz_linear_materialService() {
		return dz_linear_materialService;
	}

	public void setDz_linear_materialService(Dz_linear_materialService<Dz_linear_materialEntity> dz_linear_materialService) {
		this.dz_linear_materialService = dz_linear_materialService;
	}
